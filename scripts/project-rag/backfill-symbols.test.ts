...
import { backfillProjectRagPostgresSymbols, parseBackfillSymbolsArgs } from './backfill-symbols.js';

describe('project-rag postgres symbol backfill cli', () => {
  it('parses project and limit args', () => {
    const args = parseBackfillSymbolsArgs(['--project', 'rag-v2-dev', '--limit=200', '--offset=0']);
    expect(args).toEqual({
      project: 'rag-v2-dev',
      limit: 200,
      offset: 0,
  ...
  it('caps limit at 10000', () => {
    const args = parseBackfillSymbolsArgs(['--project', 'test', '--limit=99999']);
    expect(args.limit).toBe(10000);
  });

  it('parses dry-run flag', () => {
    const args = parseBackfillSymbolsArgs(['--project', 'test', '--dry-run']);
    expect(args.dryRun).toBe(true);
  });

  it('defaults limit to 5000 and offset to 0', () => {
    const args = parseBackfillSymbolsArgs(['--project', 'test']);
    expect(args.limit).toBe(5000);
    expect(args.offset).toBe(0);
  });

  it('requires an explicit project ref', () => {
    expect(() => parseBackfillSymbolsArgs([])).toThrow('Missing --project');
  });
  ...
  it('refuses non-dry-run writes with UNGUARDED_INDEX_MUTATION_REFUSED', async () => {
    ...
    await expect(backfillProjectRagPostgresSymbols(nonDryArgs)).rejects.toThrow(
      'UNGUARDED_INDEX_MUTATION_REFUSED'
    );
  });
});

describe('backfill-symbols security block reporting', () => {
  it('ok is false when filesSkippedBlocked > 0 per the updated logic', () => {
    // The updated ok logic: filesSkippedBlocked === 0 && (errors.length === 0 || errors.length < totalFiles)
